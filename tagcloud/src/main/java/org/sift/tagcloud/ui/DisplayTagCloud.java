...
public class DisplayTagCloud <S extends DisplayTag> extends TagCloud<S> {
	...
	/**
	 * Interface method implementation. Lays out this tag cloud using the contained DisplayTag
	 * instances.
	...
	public void layoutTagCloud() throws PlatformException {
		...
		for(DisplayTag tag : this.getTagsList()) {
			...
			int fontSize=(int)(((tag.getWeight()-low)/(high-low))*(this.getBiggestFontSize() - this.getSmallestFontSize())) + this.getSmallestFontSize();
		...
		for (int i=1;i< this.getTagsList().size();++i) {
			DisplayTag currentTag=this.getTagsList().get(i);			
			//calculate current center
			center.x=0;
			center.y=0;
			double totalWeight=0.0;
			for(int prev=0;prev< i;++prev) {
				DisplayTag prevTag = this.getTagsList().get(prev);
				center.x+= (prevTag.getBounds().getCenterX()) * prevTag.getWeight();
				center.y+= (prevTag.getBounds().getCenterY()) * prevTag.getWeight();
				totalWeight+=prevTag.getWeight();
			}
			...
			while(!done) {
				...
				for(int deg=startDeg;deg<startDeg+360; deg += DELTA_DEGREE) {
					...
					Shape candidate=moveTo.createTransformedShape(currentTag.getShape());
					Rectangle2D bound1=new Rectangle2D.Double(currentTag.getBounds().getX()+cx, currentTag.getBounds().getY()+cy,
							currentTag.getBounds().getWidth(),currentTag.getBounds().getHeight());
					...
					for(prev=0;prev< i;++prev) {
						...
						if(prev==i) {
							currentTag.setShape(candidate);
							currentTag.setBounds(candidate.getBounds2D());
							done=true;
							break;
						}
					}
